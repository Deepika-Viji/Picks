...
const ManageCalculations = () => {
  ...
  const fetchPicksModelsAndTables = async () => {
    try {
      ...
      // Fetch models
      const modelsResponse = await fetch('http://localhost:5000/api/calculate/models');
      ...
      // Fetch tables
      const tablesResponse = await fetch('http://localhost:5000/api/calculate/tables');
  ...
  const handleDeleteModel = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`http://localhost:5000/api/calculate/models/${id}`, { method: 'DELETE' });
          if (!response.ok) {
  ...
  const handleDeleteTable = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await fetch(`http://localhost:5000/api/calculate/tables/${id}`, { method: 'DELETE' });
          if (!response.ok) {
