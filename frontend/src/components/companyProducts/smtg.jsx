...
const UserProducts = () => {
  ...
  const handleEditChannel = useCallback((index) => {
    ...
    navigate('/requirements', {
      state: {
        ...state, // Include all current state
        teleportType: state.teleportType,
        asiPorts: state.asiPorts,
        selectedHardware: state.selectedHardware,
        selectedApplication: state.selectedApplication,
        ...
        remainingIP,
        remainingASI,
        networkPort: state.networkPort, // Preserve network port
        throughput: state.throughput,   // Preserve throughput
        storageType: state.storageType, // Preserve storage type
        storageCapacity: state.storageCapacity, // Preserve storage capacity
        partCode: state.partCode,       // Preserve part code
      },
    });
  ...
  const handleSubmitAllChannels = async () => {
    ...
    try {
      ...
      // Always count uncompressed outputs (remove isDecode condition)
      const { hdmiCount = 0, sdiCount = 0 } = countUncompressedOutputs(state.channels);

      let sdiPart = '';
      let hdmiPart = '';

      // HDMI outputs (HD11, HD21, etc.) - always included if present
      if (hdmiCount > 0) {
      ...
      // SDI outputs (RD11, RD14, etc.) - always included if present
      if (sdiCount > 0) {
      ...
      Swal.fire({
        ...
        // Always navigate to my-configurations after submission
