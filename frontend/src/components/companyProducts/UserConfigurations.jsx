...
const UserConfigurations = () => {
  ...
  const fetchConfigurations = useCallback(async () => {
    try {
      ...
      const response = await axios.get('http://localhost:5000/api/configurations/user', {
  ...
  const handleDeleteConfiguration = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`http://localhost:5000/api/configurations/${id}`, {
          headers: { Authorization: `Bearer ${authToken}` }
        });
  ...
  config.channels.forEach(channel => {
    ...
    navigate('/products', { 
      state: { 
        ...
        storageCapacity: config.storage.capacity,
