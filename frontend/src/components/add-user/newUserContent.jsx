...
const UserForm = () => {
  ...
  useEffect(() => {
    if (userIdFromState) {
      setLoading(true);
      axios.get(`${api.baseUrl}/api/users/${userIdFromState}`)
        .then(response => {
          setFormData({
  ...
  const checkDuplicate = async () => {
    if (formData.username !== originalUsername || formData.email !== originalEmail) {
      try {
        const response = await  axios.get(`${api.baseUrl}/api/users/check`, { params: {
    ...
    try {
      ...
      if (result.isConfirmed) {
        if (userIdFromState) {
          ...
          await axios.put(`${api.baseUrl}/api/users/${userIdFromState}`, updatedData);
          Swal.fire('Updated!', 'User has been updated.', 'success');
        } else {
          await axios.post(`${api.baseUrl}/api/users`, formData);
